...
```{r ans13}
grade1 =  grade & major == "經濟系"
grade1 <- as.integer(grade1)
grade1
grade2 = grade * grade1
grade2
meanEcon = sum(grade2)/sumEcon
meanEcon
```
...
```{r ans21}
library(lubridate)
submitTime <- ymd_hms("2017-12-28 10:28:58")
submitTimeVector <-ymd_hms(
  c("2017-12-28 10:28:58",
    "2017-12-28 10:38:42")
)
class(submitTime)
submitTime
...
```{r ans22}
twTime <- ymd_hms("2019-04-09 14:04:00", tz="Asia/Taipei")
twTime
```
...
```{r ans31}
earn = credit & grade >= 60 
earn1 = as.integer(earn)
earnedCredit = as.integer(earn1 * credit)
class(earnedCredit)
earnedCredit
```
...
```{r ans32}
score = studentID == "046S6f645" & grade & schoolYear == "100"
credit100ChosenStudent <- sum((as.integer(score))*credit)
...
```{r ans33}
score1 = studentID == "046S6f645" & grade >= 60 & schoolYear == "100"
earnedCredit100ChosenStudent <- sum((as.integer(score1))*credit)
...
```{r ans34}
score2 = studentID == "046S6f645" & credit > 0 & schoolYear == "100"
sum1 = sum((as.integer(score2)) * grade * credit)
credit1 = sum(as.integer(score2) * credit)
gpa100ChosenStudent = sum1/credit1
gpa100ChosenStudent
```
